...
`require-has-check` 是一个自定义 ESLint 规则，用于强制在对象属性检查中使用 `hasOwnProperty` 或 `Map.has`，而不是直接通过索引比较 `null`。此规则的目标是提高代码的安全性，避免由于直接索引检查带来的潜在逻辑错误和边界问题。

## 功能

- **禁止通过索引直接比较**: 自动检测代码中 `obj[key] === null` 形式的属性检查，并禁止使用。
